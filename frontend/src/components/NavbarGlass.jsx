...
function LanguageSwitcher() {
  ...
  const pick = (code) => {
    ...
    setOpen(false);
    // Soft signal so other parts of the app (e.g. apiClient header injector) can react.
  ...
  return (
    ...
    >
      <button
        ...
        aria-haspopup="true"
        aria-expanded={open}
        className="flex items-center gap-1 text-[11px] font-bold tracking-[0.1em] uppercase text-black/70 hover:text-[#E83354] transition-all hover:-translate-y-0.5"
      >
        <GlobeIcon />
        ...
        <div className="absolute right-0 top-full mt-2 w-44 bg-white border border-black/10 shadow-xl z-50 py-1">
          {LANGUAGES.map((l) => (
            <button
              key={l.code}
              onClick={() => pick(l.code)}
              className={`w-full text-left px-4 py-2 text-sm flex items-center justify-between transition-colors ${l.code === locale ? 'bg-black text-white' : 'text-black/70 hover:bg-black/5 hover:text-black'
                }`}
            >
              <span>{l.label}</span>
